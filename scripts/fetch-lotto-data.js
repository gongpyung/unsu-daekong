...
const DELAY_MS = 100;
const OUTPUT_FILE = new URL("../src/data/winning_numbers.json", import.meta.url);
const OUTPUT_FILE_PATH = fileURLToPath(OUTPUT_FILE);
...
async function fetchLottoData() {
    ...
    if (existsSync(OUTPUT_FILE)) {
        try {
            ...
            existingData = JSON.parse(await readFile(OUTPUT_FILE, "utf8"));
            console.log(`Found existing data up to round ${existingData.length}. Starting from round ${existingData.length + 1}.`);
        } catch (error) {
            console.error("Error reading existing file, starting from scratch.", error);
        }
    }
    ...
    if (newCount > 0) {
        await writeFile(OUTPUT_FILE, JSON.stringify(existingData));
        console.log(`Updated ${OUTPUT_FILE_PATH}`);
    } else {
        console.log(`Already up to date at ${OUTPUT_FILE_PATH}.`);
    }
}
